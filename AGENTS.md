...
> 8. **History** — the clock icon lists every tab you've recently closed, however you closed it, and reopens one with a click. It updates live, no refresh needed.
...
- Disposable rules narrow by a single comma-separated **Path pattern** field: `/j/*` (prefix — that path and everything under it), `/home` (exact path only), `!#inbox/` (veto — excludes any URL containing that text); blank matches only the site root. The shipped Gmail rule is `/*, !#inbox/, !#sent/`.
...
- **Settings must stay serialisable** — no functions; they have to survive `chrome.storage`. That's why disposable rules are declarative data (`urlNotContains`) rather than predicates.
...
- **The options-page "Path pattern" field is a presentation-layer encoding, not a schema change.** `DisposableRule` keeps separate `pathPrefix`/`pathExact`/`urlNotContains` fields in storage and `core/matching.ts`; only `options/draft.ts`'s `patternToField()`/`patternFromField()` collapse them, exactly as `hostnameToField()`/`hostnameFromField()` collapse `hostname`/`hostnameEndsWith`. Keep the round-trip lossless for every shipped default — there's a test.

**Grouping, pinning, sorting**

- **Domain cards group by registrable domain, not raw hostname.** `core/url.ts`'s `groupKeyOf`/`registrableDomainOf` collapse `mail.google.com` and `calendar.google.com` to `google.com` before `core/grouping.ts`'s `groupTabs()` buckets tabs, so subdomains share a card without a merge rule. `isDisposableDomain` (`core/matching.ts`) is a separate predicate kept only for its tests — don't wire it into `sortGroups()`.
- **Card and chip order: pinned (configured order), then alphabetical by displayed title, Disposable always last.** `sortGroups()` takes a `pinnedGroupOrder: Map<groupKey, earliest pinnedIndex>` from `buildPinnedPriorities()`; a card with no entry falls to the alphabetical tier via `groupDisplayTitle()` (an explicit `label` or friendly domain decides position — never `key` or tab count). `ui/render/cards.ts`'s `renderGroupCard()` mirrors those tiers *inside* one card over real-tab chips plus `PinnedPlaceholder` chips, pinned first by global `pinnedIndex` (via `matchPinnedSite()` against `pinnedMatches: PinnedMatch[]`). Don't reintroduce tab count as a sort key.
