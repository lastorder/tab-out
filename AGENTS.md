...
- **Card and chip order: Disposable, then pinned (in configured order), then alphabetical by displayed title.** `sortGroups()` takes a `pinnedGroupOrder: Map<groupKey, earliest pinnedIndex>` from `buildPinnedPriorities()`; a card with no entry in it falls straight to the alphabetical tier (via `groupDisplayTitle()`, so a card's explicit `label` or friendly-domain name decides its position, not its `key` or tab count). `ui/render/cards.ts`'s `renderGroupCard()` mirrors the same two tiers *inside* one card, over a combined list of real-tab chips and `PinnedPlaceholder` chips: pinned first (by `pinnedIndex`, from `pinnedHostnames: Map<hostname, PinnedHostnameEntry>`), then alphabetical by displayed label. Interleaved pinned sites from different cards still order correctly because `pinnedIndex` is a global position, not per-card. Don't reintroduce tab-count as a sort key — it was deliberately replaced.
