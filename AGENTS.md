...
> 10. **Your tab bar quietly reorders itself** to match the dashboard — on by default; turn it off in Settings for a manual "Sort tabs" banner. It only ever tidies the plain, unpinned tabs: your Chrome tab groups and pinned tabs are left exactly where you put them, and loose tabs are sorted within the gaps between them, never across a group.
...
- **Sorting treats Chrome groups and pinned tabs as immovable blocks — the user's tab-bar skeleton always wins.** `core/selection.ts#isMovableTab` is the single definition of "Tab Out may reposition this": not pinned (`TabInfo.pinned`), not in a group (`groupId !== -1`). `desiredTabOrder()` filters by it, and `planTabSort()` is the only thing that produces moves. It splits the movable tabs' indices into maximal *runs* of consecutive movable tabs and permutes **each run's own members** among the positions that run already occupies — so a move is always confined to one run, no fixed block's index ever changes, and a tab is never dragged *across* a group.
  - **This replaced an absolute-index loop (`move(tab, i)` for i = 0,1,2…) that shoved groups around.** With a group at index 0–1 and one loose tab at 2, it moved that loose tab to index 0 and pushed the group to 1–2 — a group visibly moving, triggered by something as ordinary as opening a pinned tab. Never go back to that shape.
  - **The consequence, on purpose: a run is sorted only against itself.** A loose tab stranded alone before a group stays before that group however late it falls in dashboard order, because moving it out would mean dragging it past the group. The tab bar's skeleton is entirely the user's; sorting can only tidy loose tabs where they already are.
  - **A non-empty move plan *is* the "out of order" signal.** There is deliberately no separate mismatch predicate: the only order `planTabSort` can reach is the one it just computed, so comparing against anything else would report a difference that could never be fixed and would re-sort forever. That is why `needsSorting()` and `chromeGroupedTabIds()` no longer exist — don't reintroduce a flat `actual` vs `desired` comparison.
