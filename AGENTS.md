...
- **A pinned site's configured `label` overrides a matching tab's own title, by hostname, not by exact URL.** `buildPinnedPriorities()`'s `byHostname` map is keyed by hostname precisely so a redirect — `https://calendar.google.com/` navigating to `https://calendar.google.com/calendar/u/0/r` — doesn't lose the "Google Calendar" label: the hostname, which is all pinning ever matches on, hasn't changed. `ui/render/cards.ts`'s `renderChip()` prefers this label over `chipLabel()`'s own title-cleanup pipeline whenever one is configured.
