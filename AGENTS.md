...
> 2. **Disposable tabs** (Gmail inbox, X home, YouTube, a spent Zoom meeting page, etc.) are in their own group at the bottom — safe to close because reopening them costs nothing.
...
- A pinned site's **Label** applies to that page *and anything under its path* — so pinning `https://calendar.google.com/` still says "Google Calendar" after Calendar redirects to a deeper URL, while pinning one Jira board only labels that board, leaving other tickets on the same Jira host with their own titles. Pin the root if you want the label to cover a whole site; pin a deep page if you want it to cover just that page.
...
- **Domain cards group by registrable domain, not raw hostname.** `core/url.ts`'s `groupKeyOf`/`registrableDomainOf` collapse `mail.google.com` and `calendar.google.com` to `google.com` before `core/grouping.ts`'s `groupTabs()` ever buckets tabs, so subdomains of one site share a card without a merge rule. `isDisposableDomain` (in `core/matching.ts`) is a separate, currently-unused-by-sorting predicate ("does any disposable rule mention this domain?") kept for its own tests; don't wire it back into `sortGroups()` without re-reading the next bullet first.
- **Card and chip order: pinned (in configured order), then alphabetical by displayed title, with Disposable always last.** `sortGroups()` takes a `pinnedGroupOrder: Map<groupKey, earliest pinnedIndex>` from `buildPinnedPriorities()`; a card with no entry in it falls straight to the alphabetical tier (via `groupDisplayTitle()`, so a card's explicit `label` or friendly-domain name decides its position, not its `key` or tab count) — except the Disposable card, which is forced last regardless, since it's a housekeeping bucket rather than content worth leading with. `ui/render/cards.ts`'s `renderGroupCard()` mirrors the pinned/alphabetical tiers *inside* one card (it has no Disposable-card concept to worry about), over a combined list of real-tab chips and `PinnedPlaceholder` chips: pinned first (by `pinnedIndex`, resolved through `matchPinnedSite()` against `pinnedMatches: PinnedMatch[]`), then alphabetical by displayed label. Interleaved pinned sites from different cards still order correctly because `pinnedIndex` is a global position, not per-card. Don't reintroduce tab-count as a sort key — it was deliberately replaced.
- **Pinning surfaces inside its own card, never gets a card of its own.** `core/grouping.ts#attachPinnedPlaceholders` never touches which card an *open* pinned tab belongs to — `groupTabs()` already placed it correctly by registrable domain. It only adds a grayed, click-to-open `PinnedPlaceholder` chip (via the returned `Map<groupKey, PinnedPlaceholder[]>`) for a pinned site with no open tab, creating an otherwise-empty card for it if none already exists. If you need a "promote this whole card to the front" feature back, that is a deliberately different, previously-removed feature — don't quietly conflate the two.
- **A pinned site claims a tab by hostname *plus path prefix* — never by hostname alone, never by exact URL.** `buildPinnedPriorities()` records a `PinnedMatch { hostname, pathPrefix, pinnedIndex, label? }` per pinned site; `matchPinnedSite(url, matches)` resolves the tab, and `ui/render/cards.ts`'s `renderChip()` prefers the matched `label` over `chipLabel()`'s own title-cleanup pipeline. Both halves of that rule are load-bearing, and each has a test: a *prefix* (not exact URL) keeps `https://calendar.google.com/` labelled "Google Calendar" after it redirects to `/calendar/u/0/r`; a *path* (not bare hostname) stops a pinned Jira board at `/jira/software/.../boards/10559` from relabelling every other tab on that Jira host — the bug that hostname-only matching originally shipped. Prefix matching respects segment boundaries (`/board` never claims `/boardroom`), and the most specific match wins, so pinning both a site root and a deep page inside it labels each tab with its closest pin.
